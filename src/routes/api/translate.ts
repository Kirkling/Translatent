      ...
      POST: async ({ request }) => {
        try {
          ...
          const system = [
            ...
            `PAGE-RELATIVE RATIO CHECK — before emitting each box, compare it to the whole page: a single dialogue block almost never exceeds ~45% of the page width or ~35% of the page height, and its area must stay well under half the page. If a box breaks those ratios you have merged several regions — split them. Likewise a box thinner than ~0.5% of the page in either axis is a measurement error — re-measure it. "capHeight" must be proportional to the box: roughly h / lines.`,
            `PAGE SANDBOX: this request covers ONE page${pageIndex && pageCount ? ` (page ${pageIndex} of ${pageCount})` : ""} in isolation. Report regions for THIS image only — never invent text from previous pages, never carry a box position over from another page. Context from earlier pages is for wording, names and tone consistency ONLY.`,
            sessionContext
              ? `SESSION CONTEXT — the running ledger for this book. Reuse the same speaker labels and keep each voice, name spelling and honorific consistent with it:\n${sessionContext}`
              : "",
